...
class HiddenTripleTechnique implements ISudokuTechnique {
  ...
  TechniqueResult? trySolveForCells(Iterable<SudokuCell> cells, Map<int, int> candidateMap, bool applyResult, Sudoku sudoku) {
    ...
    var suitableCandidates = candidateMap.keys.where((k) => candidateMap[k] == 2 || candidateMap[k] == 3);
    if (suitableCandidates.length < 3) return null;
    ...
    for (var me in candidateCellMap.entries) {
      for (var me1 in candidateCellMap.entries) {
        for (var me2 in candidateCellMap.entries) {
          if (HashSet.from({me.key, me1.key, me2.key}).length < 3) continue;
          HashSet<SudokuCell> cellSet = HashSet.from(me.value);
          cellSet.addAll(me1.value);
          cellSet.addAll(me2.value);
          if (cellSet.length == 3) {
            if (cellSet.every((c) {
              var candidateSet = HashSet.from(c.candidates);
              candidateSet.remove(me.key);
              candidateSet.remove(me1.key);
              candidateSet.remove(me2.key);
              return candidateSet.isEmpty;
            })) return null;
            if (applyResult) {
              for (var c in cellSet) {
                c.candidates.removeWhere((cc) => !{me.key, me1.key, me2.key}.contains(cc));
              }
            }
            var removedCandidatesResult = cellSet.map((c) {
              var r = SudokuCell(c.row, c.col, true, c.value);
              r.candidates = c.candidates.where((c) => c != me.key && c != me1.key && c!= me2.key).map((cc) => cc).toList();
              return r;
            });
            return TechniqueResult(successful: true, applicableCells: cellSet.toList(), removedCandidates: removedCandidatesResult.toList(), usedTechnique: HiddenTripleTechnique);
          }
        }
      }
    }
